...
Locally run  `./naive --addr=127.0.0.1 --port=1080 --proxy=https://<user>:<pass>@<domain>[:port] --log` and point the browser to a SOCKS5 proxy at port 1080.
...
Prerequisites:
* Ubuntu (apt-get install): gcc, g++, git, python2, ninja-build (>= 1.7), pkg-config, libnss3-dev, ccache (optional)


Build (output to `./out/Release/naive`):
```
git clone https://github.com/inSilo/naive_project.git
cd naive_project/src
EXTRA_FLAGS='
	target_cpu="arm64"
	clang_use_chrome_plugins=false
	clang_base_path="/home/user/Desktop/llvm-project/out/clang/release"
' ./build-with-clang.sh
```
